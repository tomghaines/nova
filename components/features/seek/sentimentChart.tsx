  ...
  useEffect(() => {
    ...
    svg
      ...
      .on('mouseover', (event, d) => {
        ...
        if (!d3.select('#sentiment-tooltip').node()) {
          d3.select('body')
            ...
            .attr(
              'class',
              'fixed p-2 bg-gray-800 text-white rounded text-xs shadow-md'
            )
            .style('opacity', 0.9)
        ...
        if (!d3.select('#ai-tooltip').node()) {
          d3.select('body')
            ...
            .attr(
              'class',
              'fixed p-2 mt-2 max-w-xs bg-gray-800 text-white rounded text-xs shadow-md'
            )
            .style('opacity', 0.9)
            .style('position', 'absolute');
        }
      ...
      .on('mousemove', (event) => {
        ...
        const priceText = matchingPriceData
          ? `$${matchingPriceData.price.toFixed(2)}`
          : 'N/A';
        ...
        d3.select('#sentiment-tooltip')
          .style(
            'left',
            `${Math.min(event.pageX + 10, window.innerWidth - 100)}px`
          )
          .style(
            'top',
            `${Math.min(event.pageY - 28, window.innerHeight - 50)}px`
          )
          .html(
            `Sentiment: ${nearestData.value.toFixed(2)}<br>Price: ${priceText}<br>Date: ${formattedDate}`
          );
        ...
        d3
          ...
          .style(
            'top',
            `${Math.min(event.pageY + 30, window.innerHeight - 70)}px`
          ).html(`
